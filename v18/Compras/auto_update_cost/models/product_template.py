...
class ProductTemplate(models.Model):
    ...
    def write(self, vals):
        # Guardar anti-recursión y recalc BoM manejados por contexto
        ...
        # Leer config UNA vez (FIX: performance + fuente unificada)
    ...
    def _recalculate_bom_costs(self):
        ...
        FIX: el message_post se hace con_company(company) para que el chatter
        se registre en el contexto correcto en entornos multi-empresa.
        """
        self.ensure_one()
        ...
        # FIX: acumular templates finales a actualizar para evitar escrituras
        # duplicadas si múltiples componentes del mismo BoM se actualizan juntos.
            ...
            for company in target_companies:
                try:
                    ...
                    if float_compare(old_cost, new_cost, precision_digits=2) != 0:
                        ...
                        # Guardar solo el último cálculo por (template, compañía)
        ...
        for (final_tmpl, final_c, company, old_cost, new_cost) in final_updates.values():
            try:
                ...
                # FIX: message_post con contexto de compañía correcto
    ...
    def _calculate_bom_cost(self, bom, company):
        ...
        FIX: aplica product_efficiency de la BoM para reflejar merma/scrap,
        igual que lo hace Odoo en su propio cálculo de costo de BoM.
        """
        total_cost = 0.0
        ...
        # Dividir por qty del producto final
        if bom.product_qty and bom.product_qty > 0:
            total_cost = total_cost / bom.product_qty

        # FIX: aplicar eficiencia de la BoM (merma/scrap)
