...
class ReportPrsExpensesPdf(models.AbstractModel):
    _name = "report.payment_register_statement.report_prs_expenses_pdf"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if report_data["mode"] == "date":
            ...
            for k, v in sorted(roots.items(), key=root_sort_key):
                ...
                v["lines"] = sorted(v["lines"], key=lambda x: (x["date"] or api.fields.Date.today(), x["payment_id"]), reverse=(wizard.order == "desc" if wizard else False))
                subs_list = []
                for sk, sv in sorted(v["subs"].items(), key=lambda kv: kv[1]["title"]):
                    sv["lines"] = sorted(sv["lines"], key=lambda x: (x["date"] or api.fields.Date.today(), x["payment_id"]), reverse=(wizard.order == "desc" if wizard else False))
                    subs_list.append(sv)
                v["subs"] = subs_list
                groups.append(v)
