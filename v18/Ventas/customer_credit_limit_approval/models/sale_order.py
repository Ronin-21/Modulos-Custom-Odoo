...
class SaleOrder(models.Model):
    ...
    def _check_salesperson_permission(self):
        ...
        if (
            ...
            or self.env.user.has_group('customer_credit_limit_approval.group_credit_approver')
        ):
            return
    ...
    def _create_review_activity_for_managers(self, excess):
        ...
        approver_group = self.env.ref(
            'customer_credit_limit_approval.group_credit_approver', raise_if_not_found=False
        )
        user_ids = set()
        if approver_group:
    ...
    def approve_credit_limit(self):
        ...
        self.ensure_one()
        if not self.env.user.has_group('customer_credit_limit_approval.group_credit_approver'):
