...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _l10n_ar_resolve_withholding_base_account(self, tax):
        self.ensure_one()
        return (
            self.company_id._l10n_ar_get_withholding_base_account()
            or (tax.company_id and tax.company_id._l10n_ar_get_withholding_base_account())
        )

    @staticmethod
    def _l10n_ar_extract_tax_ids(command_list):
        tax_ids = []
        for command in command_list or []:
            if isinstance(command, (list, tuple)) and len(command) >= 3:
                if command[0] == 6:
                    tax_ids.extend(command[2] or [])
                elif command[0] == 4:
                    tax_ids.append(command[1])
            elif isinstance(command, int):
                tax_ids.append(command)
        return tuple(sorted(set(tax_ids)))

    def _l10n_ar_get_original_base_line_specs(self):
        self.ensure_one()
        sign = -1 if self.partner_type == "supplier" else 1
        conversion_rate = self._get_conversion_rate()
        specs = []
        original_account_id = self.company_id.l10n_ar_tax_base_account_id.id or False
        for base_amount in list(set(self.l10n_ar_withholding_ids.mapped("base_amount"))):
            withholding_lines = self.l10n_ar_withholding_ids.filtered(lambda line: line.base_amount == base_amount)
            nice_base_label = ",".join(withholding_lines.mapped("name"))
            signed_base_amount = sign * base_amount
            company_base_amount = self.company_currency_id.round(signed_base_amount * conversion_rate)
            specs.append({
                "name": nice_base_label,
                "account_id": original_account_id,
                "amount_currency": signed_base_amount,
                "balance": company_base_amount,
                "tax_ids": tuple(sorted(withholding_lines.mapped("tax_id").ids)),
            })
            specs.append({
                "name": nice_base_label,
                "account_id": original_account_id,
    ...
    def _l10n_ar_line_matches_spec(self, line_vals, spec):
        return (
            line_vals.get("name") == spec["name"]
            and line_vals.get("account_id") == spec["account_id"]
            and self.currency_id.is_zero((line_vals.get("amount_currency") or 0.0) - spec["amount_currency"])
            and self.company_currency_id.is_zero((line_vals.get("balance") or 0.0) - spec["balance"])
            and self._l10n_ar_extract_tax_ids(line_vals.get("tax_ids")) == spec["tax_ids"]
        )
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        original_specs = self._l10n_ar_get_original_base_line_specs()
        cleaned_write_off_lines = []
        pending_specs = list(original_specs)
        for line_vals in payment_vals.get("write_off_line_vals", []):
            match_index = next(
                (index for index, spec in enumerate(pending_specs) if self._l10n_ar_line_matches_spec(line_vals, spec)),
                None,
            )
            if match_index is None:
                cleaned_write_off_lines.append(line_vals)
            else:
                pending_specs.pop(match_index)
        payment_vals["write_off_line_vals"] = cleaned_write_off_lines

        sign = -1 if self.partner_type == "supplier" else 1
        conversion_rate = self._get_conversion_rate()
        grouped_withholding_lines = {}
        for line in self.l10n_ar_withholding_ids:
            base_account = self._l10n_ar_resolve_withholding_base_account(line.tax_id)
            if not base_account:
                raise UserError(_(
                    "Please configure a Tax Base Account on company %s, one of its parents, or on the withholding tax company %s."
                ) % (self.company_id.display_name, line.tax_id.company_id.display_name))
            key = (line.base_amount, base_account.id)
            grouped_withholding_lines.setdefault(key, self.env["l10n_ar.payment.register.withholding"])
            grouped_withholding_lines[key] |= line

        for (base_amount, account_id), withholding_lines in grouped_withholding_lines.items():
            nice_base_label = ",".join(withholding_lines.mapped("name"))
            signed_base_amount = sign * base_amount
            company_base_amount = self.company_currency_id.round(signed_base_amount * conversion_rate)
    ...
    def _x_ar_get_prs_expense_concept_id(self):
        """Obtiene el concepto de gasto único heredable al pago."""
        self.ensure_one()
        payment_model = self.env["account.payment"]
        if "prs_expense_concept_id" not in payment_model._fields:
            return False

        try:
            concepts = self.line_ids.move_id.mapped("prs_expense_concept_id").filtered(lambda c: c)
        except Exception:
            return False

        return concepts.id if len(concepts) == 1 else False

    def _x_ar_get_prs_statement_for_journal(self, journal):
        """Retorna el estado de cuenta aplicable para el diario del pago.

        - En pago simple reutiliza el elegido en el wizard cuando coincide el diario.
        - En multi-pago, si la línea usa otro diario, busca el último estado abierto
          compatible de ese diario para no perder la funcionalidad del módulo PRS.
        """
        self.ensure_one()
        payment_model = self.env["account.payment"]
        if "prs_statement_id" not in payment_model._fields or not journal:
            return False

        auto_types = getattr(payment_model, "_PRS_AUTO_EXTRACT_TYPES", ("cash", "bank", "credit_card"))
        if journal.type not in auto_types:
            return False
        if not getattr(journal, "auto_extract_enabled", False):
            return False
        if not self.env.user.has_group("payment_register_statement_v19.group_prs_assign_payments_to_statements"):
            return False

        selected_statement = getattr(self, "prs_statement_id", False)
        if selected_statement and selected_statement.journal_id.id == journal.id:
            if "prs_state" in selected_statement._fields and selected_statement.prs_state == "closed":
                raise ValidationError(_("No se puede asignar un pago a un Estado de Cuenta cerrado."))
            return selected_statement

        statement_model = self.env["account.bank.statement"]
        domain = [("journal_id", "=", journal.id)]
        if "prs_state" in statement_model._fields:
            domain.append(("prs_state", "=", "open"))
        elif "state" in statement_model._fields:
            domain.append(("state", "not in", ("close", "closed")))
        return statement_model.search(domain, order="date desc, id desc", limit=1)

    def _x_ar_extend_payment_vals_with_prs(self, payment_vals, journal):
        """Completa vals de payment con compatibilidad PRS si el módulo existe."""
        self.ensure_one()
        payment_vals = dict(payment_vals)

        prs_statement = self._x_ar_get_prs_statement_for_journal(journal)
        if prs_statement:
            payment_vals["prs_statement_id"] = prs_statement.id

        prs_concept_id = self._x_ar_get_prs_expense_concept_id()
        if prs_concept_id:
            payment_vals["prs_expense_concept_id"] = prs_concept_id
    ...
    def _create_payment_vals_from_line_multi(self, lines_to_reconcile, line):
        ...
        payment_vals = self._x_ar_extend_payment_vals_with_prs(payment_vals, line.journal_id)
        return payment_vals
    ...
    def _create_payment_vals_from_single_custom(self, lines_to_reconcile):
        ...
        payment_vals = self._x_ar_extend_payment_vals_with_prs(payment_vals, self.journal_id)
        return payment_vals
