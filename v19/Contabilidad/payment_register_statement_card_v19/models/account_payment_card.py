...
class AccountPayment(models.Model):
    ...
    def action_post(self):
        ...
        if self.env.context.get('from_statement_reconciliation'):
            return res
        # La base solo llama _prs_create_statement_lines para diarios con auto_extract_enabled=True.
        # Los pagos con tarjeta deben generar su prs.money.flow (card_settlement) sin depender
        # de ese flag, porque los diarios puente de tarjeta típicamente no lo tienen activo.
        already_processed_ids = {
            p.id for p in self
            if p.journal_id.type in getattr(p, '_PRS_AUTO_EXTRACT_TYPES', ('cash', 'bank', 'credit_card'))
            and p.journal_id.auto_extract_enabled
        }
        for payment in self:
            if payment.id in already_processed_ids:
                continue
            if not payment.prs_card_id or not payment.company_id.prs_money_flow_enabled:
                continue
            # Verificar que hay un diario de liquidación configurado para no caer en el
            # fallback de _prs_create_statement_lines (que crearía un extracto no deseado).
            installment = payment.prs_installment_id
            config = (
                installment._prs_as_config_dict() if installment
                else payment.prs_card_id._prs_as_config_dict()
            )
            if not config.get('journal'):
                continue
