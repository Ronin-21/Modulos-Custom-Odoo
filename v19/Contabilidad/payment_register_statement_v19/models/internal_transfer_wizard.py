...
class PrsInternalTransferWizard(models.TransientModel):
    ...
    def action_confirm(self):
        ...
        try:
            ...
            # 4) Reemplazar suspense del destino con la cuenta de transferencia.
            self._replace_suspense_with_transfer_account(dst_line)

            # 5) Reconciliar apuntes de transferencia (solo mismo-empresa).
            self._reconcile_transfer_account_lines(src_line, dst_line)

            # 6) Marcar ambas líneas como comprobadas (no "Por revisar")
            for line in (src_line, dst_line):
                try:
                    line.move_id.with_context(skip_readonly_check=True).write(
                        {'checked': True}
                    )
                except Exception:
                    pass
