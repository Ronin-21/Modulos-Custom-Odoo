...
from .prs_utils import prs_is_pos, prs_vals_look_like_pos, prs_journal_uses_receivable, prs_statement_line_domain
...
class AccountPayment(models.Model):
    ...
    def _onchange_journal_id_prs_statement(self):
        for pay in self:
            ...
            if not pay.env.user.has_group(
                'payment_register_statement_v19.group_prs_assign_payments_to_statements'
            ):
                continue
    ...
    def _prs_warn_missing_memo(self):
        """Envía una única notificación de bus si algún pago no tiene Memo."""
        missing = [
            p for p in self
            if getattr(p.journal_id, 'prs_warn_missing_memo', True) and not getattr(p, 'memo', False)
        ]
        if not missing:
            return
        if len(missing) == 1:
            msg = "⚠️ El pago %s no tiene 'Memo' definido. Se recomienda completarlo." % missing[0].name
        else:
            names = ', '.join(p.name for p in missing)
            msg = "⚠️ Los siguientes pagos no tienen 'Memo': %s. Se recomienda completarlos." % names
        self.env['bus.bus']._sendone(
            self.env.user.partner_id,
            'simple_notification',
            {'title': "Campo requerido", 'message': msg, 'sticky': False, 'type': 'warning'},
        )
    ...
    def _prs_create_statement_lines(self):
        ...
        line = LineModel.create(vals)

        # ── Auto-conciliación contra el pago (si el diario lo pide) ──────────
        # Se hace ANTES de recalcular el saldo para que los diarios con
        # "Solo contabilizar conciliados" reflejen la línea ya conciliada.
        if getattr(journal, 'prs_auto_reconcile', False):
            self._prs_try_auto_reconcile_line(line)
    ...
    def _prs_try_auto_reconcile_line(self, line):
        """Cierra automáticamente la línea de extracto contra el apunte de
        recibos/pagos pendientes de este pago.

        Solo actúa cuando:
        - la línea existe, está vinculada a este pago y NO está conciliada;
        - hay exactamente un apunte outstanding del pago, sin conciliar;
        - el importe coincide exacto (sin parciales).

        Cualquier problema se loguea sin interrumpir el posteo del pago: ante la
        duda, la línea queda abierta para conciliación manual.
        """
        self.ensure_one()
        try:
            if not line or not line.exists() or getattr(line, 'is_reconciled', False):
                return
            # set_line_bank_statement_line lo aporta account_accountant (enterprise)
            if not hasattr(line, 'set_line_bank_statement_line'):
                return
            ...
            aml = self.move_id.line_ids.filtered(
                lambda l: l.account_id in outstanding_accounts
            ...
            # Importe debe coincidir exacto → si no, dejar para conciliación manual.
            company_currency = self.company_id.currency_id
            if not company_currency.is_zero(abs(aml.amount_residual) - abs(line.amount)):
                return

            line.set_line_bank_statement_line(aml.ids)
            _logger.info(
                "PRS: extracto %s auto-conciliado contra pago %s (apunte %s)",
                line.id, self.name, aml.id,
            )
        except Exception as e:
            _logger.warning(
                "PRS: no se pudo auto-conciliar el extracto del pago %s: %s",
                self.name, e,
            )
    ...
    def _get_statement_lines_for_payment(self):
        ...
        for payment in self:
            ...
            if 'payment_id' in Line._fields:
                dom = [('payment_id', '=', payment.id)]
            else:
                # Fallback: acota por diario y fecha para evitar falsos positivos
                # con pagos que comparten el mismo memo (ej. "Efectivo").
                dom = [
    ...
    def _cleanup_related_statements(self):
        ...
        for payment in self:
            label = payment._prs_get_payment_label()
            dom = prs_statement_line_domain(payment.id, label, Line._fields)
            ...
            for line in lines:
                ...
                if statement and statement.exists():
                    # Invalidar caché de line_ids antes de verificar: el unlink
                    # anterior puede haber dejado el caché del ORM desactualizado.
                    statement.invalidate_recordset(['line_ids'])
