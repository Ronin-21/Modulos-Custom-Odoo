# -*- coding: utf-8 -*-
"""
Utilidades compartidas del módulo payment_register_statement_v19.
...
def prs_statement_line_domain(payment_id, label, line_fields):
    """Domain para encontrar líneas de extracto creadas por un pago.

    Args:
        payment_id: ID entero del pago.
        label: referencia textual del pago (payment_ref / name).
        line_fields: conjunto de nombres de campo del modelo de línea (line._fields).
    """
    dom = ['|', ('payment_ref', '=', label), ('name', '=', label)]
    if 'payment_id' in line_fields:
        dom = ['|', ('payment_id', '=', payment_id)] + dom
    return dom
