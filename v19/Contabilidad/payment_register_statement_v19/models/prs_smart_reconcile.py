...
class AccountBankStatementLine(models.Model):
    ...
    def _prs_find_open_item_for_partner(self, partner, amount_abs, company, excluded_ids=None):
        ...
        if excluded_ids:
            candidates = candidates.filtered(lambda l: l.id not in set(excluded_ids))
        best = []
        for l in candidates:
            residual = getattr(l, "amount_residual", False)
            if residual is False:
    ...
    def _prs_find_payment_counterpart_aml(self, amount_abs, company, partner=None, excluded_ids=None):
        ...
        return best_aml
    def _prs_pick_counterpart_aml(self, excluded_ids=None):
        ...
        if partner:
            aml = self._prs_find_open_item_for_partner(partner, amount_abs, company, excluded_ids=excluded_ids)
            if aml:
    ...
    def _prs_smart_reconcile_run_batch(self):
        ...
        for line in self:
            ...
            if line._prs_is_reconciled():
                continue
            # ── POS receivable: dejar para conciliación manual ──────────────
            # Estas líneas provienen del cierre de POS con cuenta Por Cobrar.
            # El asiento contable ya fue creado por el cierre del POS; NO
            # creamos PBNK automático ni sugerimos conciliación inteligente.
            # El usuario debe ir a Conciliación Bancaria y seleccionar
            # manualmente el apunte existente en la cuenta Por Cobrar.
            if getattr(line, 'prs_pos_receivable', False):
                _logger.debug(
                    "Smart reconcile: omitiendo línea %s (prs_pos_receivable=True)", line.id
                )
                continue
            # ────────────────────────────────────────────────────────────────
