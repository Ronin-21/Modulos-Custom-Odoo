...
1. [Activación global](#activación-global)
2. [Activación por diario](#activación-por-diario)
3. [Flujo de Pagos (prs.money.flow)](#flujo-de-pagos-prsmoney-flow)
4. [Encadenamiento de saldos](#encadenamiento-de-saldos)
5. [Conceptos de gasto](#conceptos-de-gasto)
6. [Gastos Varios](#gastos-varios)
7. [Transferencias internas](#transferencias-internas)
8. [Diarios multi-empresa](#diarios-multi-empresa)
9. [Flujos contables](#flujos-contables)
10. [API — Métodos sobreescribibles](#api--métodos-sobreescribibles)

---

## Activación global

`Ajustes → Contabilidad → Flujo de Pagos → Flujo de Pagos`

El toggle `prs_money_flow_enabled` activa o desactiva la generación de Flujos de Pagos para la empresa actual. Controla si los registros `prs.money.flow` se crean al confirmar pagos.

Los **extractos automáticos** (`auto_extract_enabled`) funcionan de forma independiente a este toggle.
...
cp -r payment_register_statement_v19 /path/to/odoo/addons/

# Instalar
./odoo-bin -i payment_register_statement_v19 -d nombre_base -u account,account_reports,l10n_latam_check --stop-after-init

# Para actualizar una versión existente
./odoo-bin -u payment_register_statement_v19 -d nombre_base --stop-after-init
```

---

## Activación por diario
...
## Flujo de Pagos (prs.money.flow)

El modelo `prs.money.flow` registra cada movimiento de dinero como un objeto de ciclo de vida independiente. Cada pago validado en un diario habilitado genera un Flujo de Pagos.
...
| `posted` | Pago confirmado, extracto automático creado |
| `statement_created` | Extracto bancario generado, pendiente de conciliar |
| `reconciled` | Extracto conciliado contra el saldo bancario real |
| `cancelled` | Anulado (el pago fue cancelado) |
...
| `journal_id` | Many2one | Diario donde se registró el pago |
| `statement_line_id` | Many2one | Línea de extracto generada |
| `amount` | Monetary | Monto del movimiento |
| `date` | Date | Fecha del pago |
| `partner_id` | Many2one | Contacto asociado |
| `expense_concept_id` | Many2one | Concepto de gasto (ver sección siguiente) |

El Flujo de Pagos es la **unidad de trazabilidad**: permite ver de un vistazo el estado de conciliación de cada pago sin entrar al wizard de conciliación bancaria.

---

## Encadenamiento de saldos

Cuando `prs_auto_balance_statements = True` en el diario, el módulo calcula automáticamente `balance_start` y `balance_end_real` en cada Estado de Cuenta, encadenando el saldo final de uno como el saldo inicial del siguiente.

### Cómo funciona

1. Al crear un Estado de Cuenta nuevo, se busca el **último Estado cerrado** del mismo diario.
2. Su `balance_end_real` se usa como `balance_start` del nuevo Estado.
3. `balance_end_real` se calcula como `balance_start + suma de líneas conciliadas`.

### Solo contabilizar conciliados

Cuando `prs_only_count_reconciled = True`:

- El balance mostrado en el panel izquierdo del tablero bancario **solo suma líneas con estado "Comprobado"**.
- Las líneas "Por revisar" no se suman al balance visible.
- Útil para diarios de tarjetas, donde el saldo real disponible es solo lo que ya fue conciliado/acreditado.
...
Estado 001  [cerrado]
  balance_start:    $0
  balance_end_real: $10.000
       ↓ (usado como balance_start del siguiente)
Estado 002  [abierto]
  balance_start:    $10.000
  líneas:           +$5.000 (cobro A)
                    -$2.000 (gasto B)
  balance_end_real: $13.000
```

Si se modifica o elimina un Estado intermedio, el encadenamiento se recalcula en los Estados siguientes.

---

## Conceptos de gasto

El modelo `prs.expense.concept` permite clasificar los movimientos por tipo de gasto o ingreso, independientemente del plan de cuentas.

Ir a **Contabilidad → Configuración → Conceptos de Gasto PRS**.

| Campo | Descripción |
|---|---|
| `name` | Nombre del concepto (ej: "Honorarios", "Alquiler", "Cobro ventas") |
| `account_id` | Cuenta contable sugerida para este concepto |
| `company_id` | Empresa (vacío = disponible en todas) |

### Uso en los Flujos de Pagos

Cuando se registra un pago con un Concepto de Gasto, ese concepto se propaga a la línea de extracto. Permite generar reportes de flujo de fondos agrupados por concepto sin depender de las cuentas contables.

### Concepto en el contacto

El campo `prs_expense_concept_id` en el contacto (`res.partner`) actúa como valor por defecto: al crear un pago para ese contacto, el concepto se completa automáticamente.

---

## Gastos Varios

El módulo incluye un formulario de **Gastos Varios** que permite registrar egresos directos desde el diario sin crear un proveedor ni una factura.

Ir a **Contabilidad → [Diario de Caja/Banco] → Gastos Varios**.

### Campos del gasto

| Campo | Descripción |
|---|---|
| `journal_id` | Diario desde el que se registra el egreso |
| `date` | Fecha del gasto |
| `amount` | Monto |
| `expense_concept_id` | Concepto de gasto (obligatorio) |
| `account_id` | Cuenta contable de gasto |
| `memo` | Descripción libre |
| `partner_id` | Proveedor o persona (opcional) |

Al confirmar, el Gasto Varios crea directamente:
1. Un asiento contable con el gasto y la cuenta de liquidez del diario.
2. Una línea de extracto automática (si el diario tiene extractos automáticos activos).

Esto elimina el paso de crear factura → validar → registrar pago para gastos menores como viáticos, descartables, etc.
...
El wizard de **Transferencia interna** mueve fondos entre diarios de Banco o Caja, tanto dentro de la misma empresa como entre empresas distintas (cross-company).

Para las transferencias cross-company (ej. Caja sucursal BONSI PLAZA → Caja Central SIUBON S.R.L.), el wizard no filtra por `company_id` — muestra todos los diarios de tipo Banco/Caja a los que el usuario tiene acceso.

Los flujos contables para ambos casos se detallan en [Flujos contables](#flujos-contables) (Flujo 3 y Flujo 4).

---

## Diarios multi-empresa
...
## Nota de instalación Odoo 19
- Mantener el nombre técnico de la carpeta del módulo como `payment_register_statement_v19`.
- No renombrar la carpeta a `payment_register_statement_v19_v19` ni similar, porque el módulo usa referencias XML y reportes ligadas a ese nombre técnico.
