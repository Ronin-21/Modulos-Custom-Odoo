...
function _getCurrentController(env) {
    return env?.services?.action?.currentController || env?.services?.action?.currentControllerRef || null;
}
...
function _isStatementContext(env) {
    ...
    // Solo account.bank.statement (formulario de extracto): el soft_reload recarga los saldos PRS.
    // account.bank.statement.line es la vista de conciliación bancaria: ahí el botón "Conciliar"
    // navega al bank-rec widget, y el soft_reload interrumpe esa navegación dejando la vista quieta.
    return resModel === "account.bank.statement";
}
...
function _guessJournalNameFromFacets() {
    const facets = document.querySelectorAll(".o_searchview_facet, .o_search_bar .o_searchview_facet, .o_cp_searchview .o_searchview_facet");
...
function _findLabeledAmount(labelMatcher) {
    ...
    // IMPORTANTE: buscar SOLO dentro de los headers de resumen (.o_statement_summary),
    // nunca en las líneas de extracto (.o_statement_line). Si se buscara en todo el árbol,
    // un resumen "Estado de cuenta" sin monto renderizado (balance 0/vacío en extractos
    // manuales) haría que el monto más cercano encontrado fuese el de la línea siguiente,
    // sobrescribiéndolo con "$ 0,00". El monto de cada línea lo muestra el widget nativo.
    const summaries = bankRecRoot.querySelectorAll(".o_statement_summary");
    for (const summary of summaries) {
        const labelEl =
            summary.querySelector("[name='label_statement_summary'], h4, .o_statement_summary_label") ||
            summary;
        const txt = _normalizeText(labelEl.textContent || summary.textContent || "");
        if (!labelMatcher(txt)) {
            continue;
        }
        // El monto, si existe, es un hermano del label dentro del mismo resumen.
        const amounts = _findLeafAmountCandidates(summary).filter((el) => !labelEl.contains(el));
        if (amounts.length >= 1 && amounts.length <= 4) {
            return { labelEl, amountEl: amounts[amounts.length - 1], container: summary };
        }
    }
    return null;
}
