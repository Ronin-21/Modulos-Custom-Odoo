...
class Action(Action):
    ...
    def run(self, action_id, context=None):
        res = super().run(action_id, context=context)
        if res:
            actions_and_prints = []
    ...
    def load(self, action_id, context=None):
        ...
        if res:
            cids = int(
                request.httprequest.cookies.get('cids') and request.httprequest.cookies.get('cids').split('-')[0]
            ...
            remove_action -= remove_action.filtered(
                lambda x: not x.access_management_id.is_apply_on_without_company and cids not in x.access_management_id.company_ids.ids
            )
...
class Home(Home):
    ...
    def web_client(self, s_action=None, **kw):
        ...
        if not kw.get('debug') or kw.get('debug') != "0":
            ...
            elif access_management and int(cids) in access_management.company_ids.ids:
                return request.redirect('/web?debug=0')
