...
{resolution_info}""".format(operation_error=operation_error, resolution_info=resolution_info)
    ...
    def unlink(self):
        value = self.env['ir.config_parameter'].sudo().search([('key', '=', 'uninstall_simplify_access_management_v19')],
                                                              limit=1).value
        if not value:
            for rec in self:
                if rec._name:
    ...
    def create(self, vals_list):
        value = self.env['ir.config_parameter'].sudo().search([('key','=','uninstall_simplify_access_management_v19')],limit=1).value
        if not value:
            if self._name:
