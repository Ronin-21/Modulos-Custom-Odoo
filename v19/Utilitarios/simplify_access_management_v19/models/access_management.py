...
class access_management(models.Model):
    ...
    _description = "Gestión de accesos"
    ...
    hide_field_ids = fields.One2many('hide.field', 'access_management_id', 'Ocultar campo', copy=True)

    remove_action_ids = fields.One2many('remove.action', 'access_management_id', 'Ocultar acciones', copy=True)

    access_domain_ah_ids = fields.One2many('access.domain.ah', 'access_management_id', 'Acceso por dominio', copy=True)
    hide_view_nodes_ids = fields.One2many('hide.view.nodes', 'access_management_id', 'Acceso a botones/pestañas', copy=True)

    self_module_menu_ids = fields.Many2many('ir.ui.menu', 'access_management_ir_ui_self_module_menu',
                                            'access_management_id', 'menu_id', 'Menú propio del módulo',
                                            default=lambda self: self.env.ref('simplify_access_management_v19.main_menu_simplify_access_management'))
  
    total_rules = fields.Integer('Reglas de acceso', compute="_count_total_rules")

    # Chatter
    hide_chatter_ids = fields.One2many('hide.chatter', 'access_management_id', 'Ocultar chatter', copy=True)

    hide_chatter = fields.Boolean('Ocultar chatter',
                                  help="The Chatter will be hidden in all model from the specified users.")
    hide_send_mail = fields.Boolean('Ocultar enviar mensaje',
                                    help="The Send Message button will be hidden in chatter of all model from the specified users.")
    hide_log_notes = fields.Boolean('Ocultar notas internas',
                                    help="The Log Notes button will be hidden in chatter of all model from the specified users.")
    hide_schedule_activity = fields.Boolean('Ocultar programar actividad',
    ...
    disable_login = fields.Boolean('Deshabilitar inicio de sesión',help="The Users can not login if this button is chek.")

    disable_debug_mode = fields.Boolean('Deshabilitar modo desarrollador',
    ...
    company_ids = fields.Many2many('res.company', 'access_management_comapnay_rel', 'access_management_id',
                                   'company_id', 'Compañías', default=lambda self: self.env.company)

    hide_filters_groups_ids = fields.One2many('hide.filters.groups', 'access_management_id', 'Ocultar filtros/agrupaciones',
                                              copy=True)
    is_apply_on_without_company = fields.Boolean(string="Aplicar sin compañía", default=True,help="When 'Aplicar sin compañía' is selected, the rules will be applied to every company.")
