from odoo import models, SUPERUSER_ID, _, tools
...
class ir_ui_view(models.Model):
    ...
    def _get_view_cache_key(self, view_id=None, view_type='form', **options):
        """Scope cached processed views to the current user/company context.

        This module applies user-specific visibility rules during view post-processing.
        In Odoo 19 the processed arch is cached in the templates cache, so if the
        cache key stays global a view generated for one user can leak to another
        user.
        """
        base_key = super()._get_view_cache_key(view_id=view_id, view_type=view_type, **options)
        allowed_company_ids = tuple(sorted(self.env.context.get('allowed_company_ids') or self.env.companies.ids or [self.env.company.id]))
        return base_key + (self.env.uid, allowed_company_ids)
