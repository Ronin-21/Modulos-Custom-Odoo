...
from odoo.addons.advanced_web_domain_widget_v19.models.domain_prepare import prepare_domain_v2,compute_domain
...
class ir_rule(models.Model):
    ...
    def _compute_domain(self, model_name, mode="read"):
        ...
        read_value = True
        self._cr.execute(SQL("SELECT state FROM ir_module_module WHERE name='simplify_access_management_v19'"))
        ...
        value = self._cr.execute(SQL(
            """SELECT value from ir_config_parameter where key='uninstall_simplify_access_management_v19' """))
        value = self._cr.fetchone()
        if not value:
            value = self._cr.execute(SQL("""select state from ir_module_module where name = 'simplify_access_management_v19'"""))
            value = self._cr.fetchone()
